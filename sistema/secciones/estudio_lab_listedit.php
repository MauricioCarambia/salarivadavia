...
$estudios = $pdo->query("
    SELECT e.id, e.estudio, e.valor, e.laboratorio_id, l.nombre AS laboratorio
    FROM estudio_lab e
    JOIN laboratorios l ON l.id = e.laboratorio_id
    ORDER BY l.nombre ASC, e.estudio ASC
")->fetchAll(PDO::FETCH_ASSOC);
?>
        ...
        <div class="card card-info card-outline">
            ...
            <div class="card-body">
                ...
                <div class="form-group" style="max-width: 320px;">
                    ...
                    <select id="filtro_laboratorio" class="form-control">
                        <option value="">Todos</option>
                        <?php foreach ($laboratorios as $lab): ?>
                            <option value="<?= htmlspecialchars($lab['nombre']) ?>"><?= htmlspecialchars($lab['nombre']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>

                <div class="table-responsive">
                    <table id="tablaEstudios" class="table  table-hover datatable">
                        ...
                        <tbody>
                            <?php foreach ($estudios as $r): ?>
                                <tr data-id="<?= $r['id'] ?>">
                                    ...
                                    <td class=" col-laboratorio">
                                        <?= htmlspecialchars($r['laboratorio']) ?>
                                    </td>

                                    <td class=" col-estudio">
...
<script>
    $(document).ready(function () {
        ...
        // =========================
        $('#filtro_laboratorio').on('change', function () {
            let valor = $(this).val();
            table.column(0).search(valor ? '^' + $.fn.dataTable.util.escapeRegex(valor) + '$' : '', true, false).draw();
        });

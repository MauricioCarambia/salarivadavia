...
if (!$laboratorios) {
    die('<div class="alert alert-danger">No hay laboratorios cargados. Cargá uno primero en la sección "Laboratorios".</div>');
}

$lab_id = $_GET['lab_id'] ?? '';

$labIds = array_column($laboratorios, 'id');

// Ya no elegimos un laboratorio por defecto: hasta que el usuario no
// seleccione uno explícitamente, no se muestran estudios para presupuestar.
if ($lab_id === '' || !in_array((int) $lab_id, $labIds, true)) {
    $lab_id = '';
}
...
<style>
    ...
    /* el "rengloncito" de arriba de la card (card-outline card-secondary)
       también pasa a celeste cuando el estudio está seleccionado */
    .item-estudio.activo.card-outline {
    ...
    #barra_resumen.resaltado {
        background-color: #b8ecf5;
        border-color: #17a2b8;
        animation: pulseResumen 0.5s ease-out;
    }

    @keyframes pulseResumen {
        0% { box-shadow: 0 0 0 0 rgba(23, 162, 184, 0.6); }
        100% { box-shadow: 0 0 0 14px rgba(23, 162, 184, 0); }
    }
</style>
<div class="row">
    <div class="col-12">
        ...
        <div class="card card-info card-outline">
            ...
            <div class="card-header d-flex  align-items-center">
                <h3 class="card-title">
                    ...
                    <a href="./?seccion=estudio_lab_new&lab_id=<?= $lab_id ?>&nc=<?= $rand ?>" class="btn btn-info btn-sm">
                    ...
                    <a href="./?seccion=laboratorios&nc=<?= $rand ?>" class="btn btn-secondary btn-sm">
                        <i class="fa fa-hospital"></i> Laboratorios
                    </a>
                </h3>
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    <label>Laboratorio a presupuestar <span class="text-danger">*</span></label>
                    <select id="selector_laboratorio" class="form-control">
                        <option value="" <?= $lab_id === '' ? 'selected' : '' ?>>-- Seleccionar laboratorio --</option>
                        <?php foreach ($laboratorios as $lab): ?>
                            <option value="<?= $lab['id'] ?>" <?= ((int) $lab['id'] === (int) $lab_id) ? 'selected' : '' ?>>
                ...
                <?php if (!$labSeleccionado): ?>

                    <!-- SIN LABORATORIO SELECCIONADO -->
                    <div class="alert alert-warning text-center">
                        <i class="fa fa-exclamation-triangle"></i>
                        Elegí un laboratorio arriba para ver y presupuestar sus estudios.
                    </div>

                <?php else: ?>

                <!-- TOTAL -->
                <div class="alert alert-primary d-flex justify-content-between align-items-center" id="barra_resumen">
                    <div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        // =========================
        $('#selector_laboratorio').change(function() {
            window.location.href = './?seccion=estudios_laboratorio&lab_id=' + $(this).val() + '&nc=<?= $rand ?>';
        });
        ...
        // =========================
        // RESALTAR BARRA SUPERIOR
        // =========================
        function resaltarResumen() {

            let barra = $('#barra_resumen');

            barra.removeClass('resaltado');

            // 🔥 forzar reflow para poder re-disparar la animación en clicks seguidos
            void barra[0].offsetWidth;

            barra.addClass('resaltado');

            clearTimeout(barra.data('timeoutId'));

            let timeoutId = setTimeout(() => barra.removeClass('resaltado'), 600);

            barra.data('timeoutId', timeoutId);
        }
    ...
    `;
        ...
        async function imprimirTicketLab(items, total) {
            ...
            try {
                ...
                contenido.push(center(<?= json_encode(mb_strtoupper($labSeleccionado['nombre'] ?? '')) ?>));
                <?php if (!empty($labSeleccionado['direccion'])): ?>
                contenido.push(center(<?= json_encode($labSeleccionado['direccion']) ?>));
                <?php endif; ?>
                <?php if (!empty($labSeleccionado['telefono'])): ?>
                contenido.push(center(<?= json_encode('TEL: ' . $labSeleccionado['telefono']) ?>));
                <?php endif; ?>

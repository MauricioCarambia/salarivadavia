...
/* ============================
   TURNOS (30 días antes / 30 días después de hoy)
============================ */
$turnosInicio = date('Y-m-d', strtotime('-30 days'));
$turnosFin = date('Y-m-d', strtotime('+30 days'));
?>
...
<div class="col-md-12">
            ...
            <form method="POST">
                <div class="row align-items-end">
                    <div class="col-md-3 form-group">
                        <label>Vista</label>
                        <select name="estadisticas" class="form-control" id="selectVistaEstadisticas">
                            ...
                            <option value="turnos" <?= $estadisticas == 'turnos' ? 'selected' : '' ?>>Turnos</option>
                        </select>
                    </div>
                    <div class="col-md-3 form-group grupo-fecha-filtro">
                        <label>Desde</label>
    ...
    <!-- ===================== TURNOS (mes actual + anterior) ===================== -->
    <?php if ($estadisticas === 'turnos'): ?>
        <div class="card card-outline card-primary">
            <div class="card-header">
                <strong>Turnos dados — <?= date('d/m/Y', strtotime($turnosInicio)) ?> al <?= date('d/m/Y', strtotime($turnosFin)) ?></strong>
                <span class="text-muted small">(30 días antes y 30 días después de hoy)</span>
            </div>
            <div class="card-body table-responsive">

                <table id="tablaTurnosStats" class="table table-bordered table-hover table-sm" style="width:100%">
                    <thead class="thead-dark">
                        <tr>
                            ...
                            <th>Empleado que dio el turno</th>
                            <th>Sobreturno</th>
                            <th>Fecha y hora en que se dio</th>
                        </tr>
                    </thead>
                    <tbody></tbody>
                </table>
...
<script>
    ...
    // Ocultar filtro Desde/Hasta cuando la vista es "Turnos" (usa un rango fijo)
    (function() {
        var select = document.getElementById('selectVistaEstadisticas');
        var grupos = document.querySelectorAll('.grupo-fecha-filtro');

        function actualizarVisibilidad() {
            var mostrar = select.value !== 'turnos';
    ...
    <?php if ($estadisticas === 'turnos'): ?>
        // Esperamos a que cargue todo el documento (incluida la librería DataTables,
        // que se incluye al final del HTML) antes de inicializar la tabla.
        $(document).ready(function() {
            $('#tablaTurnosStats').DataTable({
                serverSide: true,
                processing: true,
                ajax: 'ajax/estadisticas_turnos_dt.php',
                order: [
                    [3, 'desc']
                ],
                language: {

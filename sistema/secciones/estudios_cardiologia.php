            ...
            <div class="card-body">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-3 d-flex align-items-end justify-content-end">
                        <button class="btn btn-secondary btn-sm rounded" id="btn_limpiar_filtros">
                    ...
                    <table class="table  table-hover table-striped datatable" style="width:100%">
                        ...
                        <thead class="thead-dark">
                            <tr>
                                ...
                                <th>Dirección</th>
                                <th>Obra Social</th>
                                <th style="max-width:150px">Estudio</th>
                                <th class="text-right" style="white-space:nowrap">Pago Médico</th>
                                <th class="text-right" style="white-space:nowrap">Cobrado</th>
                                <th>Día Turno</th>
                                <th>Aviso</th>
...
<!-- OJO: esta app usa DataTables 1.13.8 (vendor/datatables), así que la
     extensión Responsive tiene que ser la rama 2.x (la 3.x/4.x requiere
     DataTables 3+ y no hace nada con esta versión).
     defer: el core de DataTables recién se carga al final del layout
     (index.php), así que estos scripts deben ejecutarse después de que
     termine de parsear todo el documento, o intentan extender
     $.fn.dataTable antes de que exista. -->
<script defer src="https://cdnjs.cloudflare.com/ajax/libs/datatables.net-responsive/2.5.0/dataTables.responsive.min.js"></script>
<script defer src="https://cdnjs.cloudflare.com/ajax/libs/datatables.net-responsive-bs4/2.5.0/responsive.bootstrap4.min.js"></script>
<script>
    $(document).ready(function() {
        ...
        // INIT DATATABLE
        // =========================
        // "responsive" oculta las columnas menos importantes detrás de un
        // botón "+" cuando no entran todas en el ancho disponible, en vez
        // de desarmar la tabla (son muchas columnas: 14).
        let table = $('.datatable').DataTable({
            ...
            columnDefs: [
                { responsivePriority: 1, targets: [0, 2, 13] }, // check, Paciente, Acciones
                { responsivePriority: 2, targets: [1, 8, 9, 12] }, // Fecha, Estudio, Pago, Aviso
                { responsivePriority: 3, targets: [3, 10] }, // Documento, Cobrado
                { responsivePriority: 4, targets: [4, 11] }, // Celular, Turno
                { responsivePriority: 5, targets: [7] }, // Obra Social
                { responsivePriority: 6, targets: [5, 6] } // Fecha Nac, Dirección (se ocultan primero)
            ],
            language: {
        ...
        $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
            ...
            let fechaDesde = $('#filtro_fecha_desde').val();
            let fechaHasta = $('#filtro_fecha_hasta').val();
            ...
            // FILTRO FECHA (rango desde/hasta)
        ...
        // Ejecutar filtros
        $('#filtro_fecha_desde, #filtro_fecha_hasta, #filtro_no_avisados, #filtro_estudio').on('change', function() {
            table.draw();
        });
        ...
        $('#btn_limpiar_filtros').on('click', function() {
            ...
            $('#filtro_estudio').val('');
            $('#filtro_no_avisados').prop('checked', false);

            $('#check_all').prop('checked', false);
            $('.checkbox_cardio').prop('checked', false);
            calcularTotal();
...
<tr>
    ...
    <th class="col-pago" style="text-align:right;">Pago</th>
</tr>
`;
...
<tr>
    ...
    <td>${f.documento}</td>
    <td>${f.celular}</td>
    <td class="col-estudio">${f.estudio}</td>
    <td class="col-pago" style="text-align:right;">$ ${f.valor.toLocaleString('es-AR')}</td>
</tr>
`;
    });
...
<div class="total">
    ...
    <span style="white-space:nowrap;">$ ${total.toLocaleString('es-AR')}</span>
</div>

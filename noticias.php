...
// LEFT(texto, 400) alcanza para el resumen más largo que se muestra (320
// caracteres del destacado) sin traer el cuerpo completo de cada artículo.
$offset = ($pagina - 1) * $porPagina;
$columnas = "id, titulo, imagen, created_at, LEFT(texto, 400) AS texto";
if ($busqueda !== '') {
    $stmt = $pdo->prepare("SELECT $columnas FROM articulos WHERE titulo LIKE ? OR texto LIKE ? ORDER BY created_at DESC LIMIT $porPagina OFFSET $offset");
    $stmt->execute([$like, $like]);
} else {
    $stmt = $pdo->prepare("SELECT $columnas FROM articulos ORDER BY created_at DESC LIMIT $porPagina OFFSET $offset");
    $stmt->execute();
}
